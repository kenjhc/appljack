...
const loadMapping = async (jobpoolid) => {
    return new Promise((resolve, reject) => {
        ...
        tempConnection.query(query, [jobpoolid], (err, results) => {
            ...
            console.log(`Mapping loaded for jobpoolid ${jobpoolid}:`, mapping);
        ...
        return new Promise((resolve, reject) => {
            ...
            parser.on('text', (text) => {
                if (currentItem && currentTag) {
                    ...
                    let fullPath = currentPath.join('.');

                    // Simplify the path by removing leading 'source.'
                    if (fullPath.startsWith('source.')) {
                        fullPath = fullPath.substring(7);
                    }

                    if (tagToPropertyMap[fullPath]) {
                        let propertyName = tagToPropertyMap[fullPath];
                        currentItem[propertyName] = (currentItem[propertyName] || '') + trimmedText;
                        console.log(`Mapped ${fullPath} to ${propertyName} with value: ${trimmedText}`);
                    } else {
                        console.log(`Unmapped path ${fullPath} with value: ${trimmedText}`);
                    }
                }
            });
            ...
            parser.on('cdata', (cdata) => {
                if (currentItem && currentTag) {
                    ...
                    if (tagToPropertyMap[fullPath]) {
                        ...
                        console.log(`Mapped ${fullPath} to ${propertyName} with CDATA value: ${cdata}`);
                    } else {
                        console.log(`Unmapped path ${fullPath} with CDATA value: ${cdata}`);
                    }
                }
            });
            ...
            parser.on('closetag', (nodeName) => {
                ...
                if (nodeName === 'job' || nodeName === 'doc') {
                    ...
                    if (!currentItem.job_reference) {
                        console.warn(`Skipping item due to missing job_reference: ${JSON.stringify(currentItem)}`);
                    } else {
                        console.log('Parsed job item:', currentItem); // Log the current item when job tag is closed
...
const singleFilePath = `/chroot/home/appljack/appljack.com/html${envSuffix}/feeddownloads/8215880437-1394632526.xml`; // Adjust the file path as necessary
