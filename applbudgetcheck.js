  ...
  try {
    ...
    for (const feed of feeds) {
      ...
      if (monthlyTotal >= parseFloat(feed.budget)) {
        await connection.query('UPDATE applcustfeeds SET status = ? WHERE feedid = ?', ['capped', feed.feedid]);
        console.log(`Monthly status changed to 'capped' for feed ID ${feed.feedid}`);
      } else {
        await connection.query('UPDATE applcustfeeds SET status = ? WHERE feedid = ?', ['active', feed.feedid]);
        console.log(`Monthly status changed to 'active' for feed ID ${feed.feedid}`);
      }
      console.log(`Feed ID ${feed.feedid}: Monthly total is ${monthlyTotal}, budget is ${feed.budget}`);

      // Daily budget check (regardless of monthly budget status)
      if (feed.dailybudget !== null && feed.dailybudget !== '' && !isNaN(parseFloat(feed.dailybudget))) {
