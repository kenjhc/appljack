...
async function fetchAllFeedsWithCriteria() {
    return new Promise((resolve, reject) => {
        poolXmlFeeds.query('SELECT acf.*, ac.jobpoolid, acf.cpc as feed_cpc FROM applcustfeeds acf FORCE INDEX (idx_status) JOIN applcust ac ON ac.custid = acf.custid WHERE acf.status = "active";', (error, results) => {
            if (error) {
...
async function fetchCustIdFromApplCustFeeds(feedid) {
    return new Promise((resolve, reject) => {
        poolXmlFeeds.query('SELECT custid FROM applcustfeeds WHERE feedid = ?', [feedid], (error, results) => {
            if (error) {
                reject(error);
            } else {
...
async function fetchJobPoolIdFromApplCust(custid) {
    return new Promise((resolve, reject) => {
        poolXmlFeeds.query('SELECT jobpoolid FROM applcust WHERE custid = ?', [custid], (error, results) => {
            if (error) {
                reject(error);
            } else {
    ...
    try {
        ...
        jobsData.forEach(job => {
            ...
            if (job.hasOwnProperty('job_reference')) {
                let customUrl = `https://appljack.com/applpass.php?c=${encodeURIComponent(custid)}&f=${encodeURIComponent(feedid)}&j=${encodeURIComponent(job.job_reference)}`;
