...
const moveDuplicateRecordsInBatches = (batchSize) => {
  ...
  const processBatch = () => {
    ...
    pool.query(duplicateQuery, (error, results) => {
      ...
      logMessage(`Batch ${batchNumber}: Duplicate IDs: ${duplicateIds}`, logFilePath);
      logToDatabase( 
        "warning",
        "applcleaneventstime.js",
      ...
      pool.getConnection((err, connection) => {
        ...
        connection.beginTransaction((transactionError) => {
          if (transactionError) { 
            logMessage(`Error starting transaction: ${transactionError.message}`, logFilePath);
            logToDatabase( 
              "error",
              "applcleaneventstime.js",
          ...
          connection.query(insertQuery, (insertError, insertResults) => {
            if (insertError) { 
              logMessage(`Error executing insert query: ${insertError.message}`, logFilePath);
              logToDatabase( 
                "error",
                "applcleaneventstime.js",
            ...
            logMessage(`Batch ${batchNumber}: Inserted ${insertResults.affectedRows} rows into appleventsdel.`, logFilePath);
            logToDatabase( 
              "success",
              "applcleaneventstime.js",
            ...
            connection.query(deleteQuery, (deleteError, deleteResults) => {
              if (deleteError) { 
                logMessage(`Error executing delete query: ${deleteError.message}`, logFilePath);
                logToDatabase( 
                  "error",
                  "applcleaneventstime.js",
              ...
              logMessage(`Batch ${batchNumber}: Deleted ${deleteResults.affectedRows} rows from applevents`, logFilePath);
              logToDatabase( 
                "success",
                "applcleaneventstime.js",
              ...
              connection.commit((commitError) => {
                if (commitError) { 
                  logMessage(`Error committing transaction: ${commitError.message}`, logFilePath);
                  logToDatabase( 
                    "success",
                    "applcleaneventstime.js",
